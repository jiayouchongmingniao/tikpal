...
- Switching from one provider to another uses the same release and readiness gates as first entry: keep external intakes closed, show `Opening` until the target has a real provider page and window, and keep the prior provider active only until the target is ready.
