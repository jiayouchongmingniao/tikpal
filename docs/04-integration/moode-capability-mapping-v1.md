...
| `/api/v1/web-mode/settings` | `PATCH` | Persists Explore proxy settings in `.tikpal/web-mode-settings.json`. The current development default is HTTP `http://192.168.10.103:7897`; persisted settings override it and must be updated when the proxy host's LAN address changes. Console changes debounce-save automatically. |
