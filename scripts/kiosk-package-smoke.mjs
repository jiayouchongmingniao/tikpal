...
`,
  ...
  assert(stylesSource.includes(".screen-saver-wake-hint") && !stylesSource.includes("animation: screen-saver-wake-hint 3.2s ease forwards;"), "Screen sleep should keep a subtle touch-to-wake hint visible");
  ...
  assert(appSource.includes('isSceneSource && style === "now_playing"') && appSource.includes("screen-saver-scene") && appSource.includes("activeSceneVideo.thumbnailSrc") && stylesSource.includes(".screen-saver-scene") && stylesSource.includes(".screen-saver-scene-identity"), "scene sound should preserve a low-power scene identity instead of the generic Now Playing card");
  assert(ambientScreenSource.includes('aria-label={t("ambient.screen")}') && ambientScreenSource.includes("data-ambient-scene-gallery-preview") && stylesSource.includes(".ambient-transport-scene-gallery.has-thumbnail") && stylesSource.includes("border: 1px solid color-mix(in srgb, var(--theme-text) 46%, var(--room-accent))"), "Ambient should localize its screen label and give the active scene thumbnail a restrained visible frame");
  assert((i18nSource.match(/"ambient\.screen"/g)?.length ?? 0) === 7, "Ambient screen accessibility label should be translated for all seven locales");
  assert(ambientScreenSource.includes("const sceneSoundEnabledForVideo = Boolean(video.audioSrc);") && ambientScreenSource.includes("sceneSoundEnabled: sceneSoundEnabledForVideo") && serverSource.includes("function resolveSceneVideoSoundEnabled") && serverSource.includes("if (sceneVideo.visualOnly) return false;") && serverSource.includes("stopScene = !sceneSoundEnabled && current.sceneSoundEnabled;"), "visual-only scenes should preserve the active source and release an earlier Scene Sound, including cached client requests");
  ...
  assert(sidePanelSource.includes("resolveProviderCardStatus"), "Explore side panel should centralize provider status labels");
