...
With `TIKPAL_WEB_MODE_ONBOARD_AUTO_FOCUS=1`, focus on text-like provider fields (`text`, `search`, URL, email, password, telephone, number, `textarea`, textbox roles, and editable content) explicitly shows Onboard. The same rule covers local physical kiosk and Console fields. Blur, form submit, and single-line Enter hide it; moving between text fields keeps it visible, while checkboxes, radio controls, selectors, ranges, and buttons never summon it. A LAN/macOS browser rendering the full UI through `http://<pi-ip>:4173/` cannot control Onboard on the Pi. The launcher binds Onboard to the kiosk user's DBus session and uses its `Show`, `Hide`, and `ToggleVisible` methods; Hide keeps the process resident. The right panel exposes one compact icon-only toggle, and Console has no duplicate Keyboard button. Provider Chromium also uses `--disable-hang-monitor`. Returning to Tikpal hides Onboard, gives provider and side-panel processes 200ms to exit normally, then force-terminates any remainder instead of waiting on a `Page Unresponsive` dialog or leaving the right panel over the kiosk.
