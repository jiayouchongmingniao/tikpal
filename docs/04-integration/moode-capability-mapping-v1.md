...
| Playback source / renderer | Player top state, source workspace | Six visible frontstage tabs: Library, Radio, Spotify, AirPlay, Bluetooth, DLNA; internal Audio remains backend/status truth. |
...
| `/api/v1/remote/catalog` | `GET` | Safe portable-controller catalog: allowed action ids, playback modes, source targets, source summaries, room-mode presets, scene videos, and Hi-Fi EQ presets. |
