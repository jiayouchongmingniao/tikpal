...
try {
  ...
  let result=await post(action);assert.equal(result.status,200,JSON.stringify(result));assert.equal(result.body.panelMode,'collapsed');assert.ok(readFileSync(path.join(root,'calls'),'utf8').includes('panel-lock-timeout=5'),'panel layout waits for an overlapping Guard inspection');
  ...
  writeFileSync(path.join(root,'audio-fail'),'1');
  assert.notEqual((await post({type:'close',closeRequestId:'fail-close'})).status,200);
  assert.equal((await get()).activeProvider,base.activeProvider,'unconfirmed audio must not clear active provider');
  rmSync(path.join(root,'audio-fail'));
  ...
  const close=post({type:'close',closeRequestId:'close-fixture'});await first;assert.equal((await close).status,200);
  const calls=readFileSync(path.join(root,'calls'),'utf8').trim().split('\n');
  assert.ok(calls.lastIndexOf('close-audio')<calls.lastIndexOf('panel-done'),'audio runs before panel layout completes');
  assert.ok(calls.lastIndexOf('close')>calls.lastIndexOf('panel-done'),'window close still follows layout');
  ...
  assert.notEqual((await post({type:'close',closeRequestId:'old-close',panelSessionId:'open-1'})).status,200,'old close rejected after re-entry');
  assert.equal((await get()).activeProvider,base.activeProvider);
