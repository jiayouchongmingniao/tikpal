...
`,
  ...
  assert(ambientScreenSource.includes("data-ambient-standby-status") && !ambientScreenSource.includes("ambient-standby-status ${clockVisible ?") && stylesSource.includes("left: max(38px, env(safe-area-inset-left));") && stylesSource.includes("top: 144px;") && !stylesSource.includes(".ambient-standby-status.has-clock") && stylesSource.includes(".ambient-screen.is-hud-visible .ambient-standby-status"), "Ambient standby should keep a fadeable identity chip away from the clock at every viewport");
