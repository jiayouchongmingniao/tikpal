...
Tikpal-owned kiosk UI, Explore side panel, portable Remote, and local Explore error pages share the device language preference from `.tikpal/ui-preferences.json`. Supported UI languages are English, Chinese, German, Italian, Korean, Japanese, and Spanish; Onboard/Fcitx starts in English on the physical kiosk, while the Onboard language key toggles directly between English and the selected UI language's input method. Ambient clock copy follows the same language: the date uses a stable `MM/DD weekday` shape such as `08/03 星期一`, and the weak scene caption translates weather, room mood, and daypart with visible separators such as `雨天 · 放松 · 早晨`; when `/api/v1/scene/context` has current weather temperature, it appends rounded Celsius such as `雨天 27°C · 放松 · 早晨`. Settings -> Font also syncs the selected font preset into device preferences so Onboard keycap labels use the same visual family as the kiosk UI. The Onboard language key keeps a small runtime pair state so touch users can switch efficiently without stepping through every installed language.
...
For local Settings -> Link -> Explore Proxy, the Proxy URL saves automatically, but the Proxy On/Off switch first asks the user to confirm the target state. Only confirmation persists the new switch state and immediately restarts the system; cancelling leaves the saved proxy state unchanged. The guarded Remote action remains an immediate runtime change. This is the current Console contract.
...
Ambient background videos are discovered from `public/assets/*.mp4` and OTA-managed `public/assets/scenes/*.mp4` through `GET /api/v1/media/background-videos`. Scene changes mount the next video layer and crossfade it after it becomes drawable; paused playback stays frozen on the new scene frame, while playing playback resumes after alignment. `FlameScene` keeps the visual loop smooth with a preloaded standby slot, retries video stalls, and falls back to the static logo surface when needed.

Scene MP4s are permanently muted. `SceneAudioTransport` is the only browser-owned Scene Sound path: it loops the independently versioned Ogg `audioSrc`, applies its scene `audioGainDb` together with `system.volume.percent`, and depends on the kiosk Chromium ALSA route reaching the physical USB output. Video loop handoffs, scene changes, and static-video fallback therefore cannot restart or interrupt the ambient audio timeline. The web server serves scene media with byte-range support, and Ambient refreshes the catalog every 30 seconds and whenever the page becomes visible so newly applied scene OTA packages join the controls without a page reload.
