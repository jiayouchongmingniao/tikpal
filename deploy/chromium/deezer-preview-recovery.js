(() => {
  ...
  const cancel = () => { clearTimeout(timer); timer = undefined; pending = undefined; watchUntil = 0; failures.clear(); revision++; };
  ...
  function check() {
    ...
    const now = Date.now();
    for (const [path, until] of failures) if (until < now) failures.delete(path);
