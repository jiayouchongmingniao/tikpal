...
Tikpal-owned kiosk UI, Explore side panel, portable Remote, and local Explore error pages share the device language preference from `.tikpal/ui-preferences.json`. Supported UI languages are English, Chinese, German, Italian, Korean, Japanese, and Spanish; Onboard/Fcitx defaults follow the selected language on the physical kiosk. Ambient clock copy follows the same language: the date uses a stable `MM/DD weekday` shape such as `08/03 星期一`, and the weak scene caption translates weather, room mood, and daypart with visible separators such as `雨天 · 放松 · 早晨`; when `/api/v1/scene/context` has current weather temperature, it appends rounded Celsius such as `雨天 27°C · 放松 · 早晨`. Settings -> Font also syncs the selected font preset into device preferences so Onboard keycap labels use the same visual family as the kiosk UI. The Onboard language key keeps a small runtime cycle state so touch users can move through EN / Chinese / German / Italian / Korean / Japanese / ES without bouncing on transient Fcitx state.
