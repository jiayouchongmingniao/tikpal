...
With `TIKPAL_WEB_MODE_ONBOARD_AUTO_FOCUS=1`, provider input clicks and focus events automatically call the existing Keyboard action. Onboard is mapped to `TIKPAL_WEB_MODE_ONBOARD_WINDOW`, moved to `TIKPAL_WEB_MODE_ONBOARD_POSITION`, and raised above Chromium; the right-panel Keyboard button remains a fallback. Provider Chromium also uses `--disable-hang-monitor`. Returning to Tikpal gives provider processes 200ms to exit normally, then force-terminates any remainder instead of waiting on a `Page Unresponsive` dialog.
