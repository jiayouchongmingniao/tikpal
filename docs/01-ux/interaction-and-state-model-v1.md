...
| Vertical drag in left ambient zone | Ambient | Live display brightness adjustment through DDC/CI when available. The overlay follows the hand immediately, backend writes are coalesced to the final resting value, and the overlay auto-dismisses after about three idle seconds. |
| Vertical drag in right ambient zone | Ambient | Live volume adjustment against moOde volume state. The overlay follows the hand immediately, backend writes are coalesced to the final resting value, and the overlay auto-dismisses after about three idle seconds. |
| Non-Hi-Fi Ambient center controls | Ambient HUD visible | Scene Gallery, Focus/Calm/Sleep label plus intent, lightweight source picker, and Scene Sound mute/unmute. The strip width should follow its content rather than a fixed playback-control width. |
...
- Visible source status labels are shared across Ambient, Player, and Console: `Active` for the current selected source, `Connected` for a live external sender, `Connecting` for an in-flight handoff, `Ready` for a selectable/open receiver, and `Unavailable` only when the backend reports no usable control path. Internal states such as `blocked` must not appear as button labels.
- Selecting `Spotify`, `AirPlay`, `Bluetooth`, or `DLNA` enters the same external handoff state in Player and Hi-Fi: hide the normal source choices, show a `Connecting` card with the advertised receiver name when available, keep polling the shared source state, and close only when the source is ready or the handoff times out and rolls back to the previous playable source. In Focus/Calm/Sleep, the picker closes immediately after the handoff starts and a lower-left source pill carries the connecting or connected state so the scene video stays immersive.
...
- The Scene Gallery entry stays inside the temporary HUD. It opens a modal gallery of Focus, Calm, and Sleep scenes, ordered by room mode then catalog order. A card selects its scene; selecting a card from another room mode changes the room mode and its scene together. The gallery closes after a selection, Escape, the close control, or a backdrop tap. It must not expose queue or playlist state.
- A scene manifest entry may provide `thumbnailSrc`. The API accepts only an existing image below `public/assets/scenes` with an approved image extension; a missing or invalid thumbnail keeps the scene selectable and renders the gallery fallback treatment instead.
...
- When the clock is visible, the weak Ambient caption may include daypart, scene, and weather/location context from `/api/v1/scene/context`, but it remains decorative copy and must not change room mode, source, timer, or Auto Night behavior. The date line follows the selected UI language while keeping a stable `MM/DD weekday` rhythm, and the caption must use localized weather / mood / daypart words with visible separators, for example `雨天 · 放松 · 早晨`; rounded Celsius may appear only when weather data includes temperature.
...
- Hi-Fi uses the larger playback center controls: playback mode, source picker, previous/next, play/pause, favorite, and lyrics. Entering Hi-Fi preserves the current visible source, including Library, Radio, Spotify, AirPlay, Bluetooth, and DLNA; it must not restore over a currently active source just because `audio.rememberedSource` points somewhere else. Restore is reserved for coming back from Scene Sound or for app/API startup while persisted room mode is already Hi-Fi, so a late-arriving `rememberedSource` can still recover a `Not Playing` room without interrupting a live source. During that startup restore, the first state visible to the kiosk should already reflect the recovered Library/Radio playback instead of a stale `stopped` snapshot. While Hi-Fi remains active, the backend should also recover remembered Library/Radio playback if MPD falls back to `stopped` or the wrong source; an explicit `paused` state and external-source waiting states are user intent and must not be auto-resumed. Hi-Fi source picker selections should release their pending UI as soon as backend state confirms the selected source, so switching from AirPlay to Bluetooth or back to Library never leaves the source button disabled. Library memory follows the actual local song after manual selection, playlist play, next, and previous; current-track checks may match by queue file path or by the remembered track's title / artist / duration so an internal queue id does not trigger a duplicate Library restore. Radio memory follows the final station that actually plays, even after the user returns to Library. Returning to either music source from Focus/Calm/Sleep should keep the current source without showing a blocking overlay.
- Hi-Fi defaults to centered now-playing artwork and metadata. When the lyrics toggle is on and `lyrics.status === "ready"` with at least one non-empty line, the main visual may switch to a shared cover-plus-lyrics wall; Radio, Bluetooth, and AirPlay all use this same wall instead of source-specific lyric layouts. Radio lyrics use `local_playback` metadata when the stream exposes a real title / artist, Bluetooth uses `bluetooth_input`, and AirPlay uses `airplay_input`. The compact rolling lyrics ticker is reserved for Focus/Calm/Sleep and must not appear under the Hi-Fi wall. The wall may include a decorative CSS-only mini EQ centered under the cover art, progress line, elapsed/duration readout, and capability-gated playback controls. When the lyrics wall is visible and the HUD is hidden, a decorative previous/play-next cluster appears for three seconds; each button opens Player rather than sending a transport command, keeping real playback control in one surface. That mini EQ should feel alive through deterministic per-bar timing and amplitude variation, but it must not claim to be real spectrum data and must pause with playback. While `lyrics.status === "recognizing"`, the centered composition shows one localized identification line; external inputs name AirPlay, Bluetooth, or DLNA, while local playback stays generic. `not_found` and `error` keep the title / artist fallback but show no failure label. When lyrics are unavailable, empty, or explicitly hidden, it must return to centered now-playing and hide lyrics surfaces. All Hi-Fi visible labels and accessibility names follow the selected UI language. Old kiosk hidden-state storage is auto-restored once so a stale `lyricsVisible=false` value does not suppress newly ready lyrics forever. The centered fallback must still show a non-interactive playing-state motion cue that is visible at low volume and does not depend on audio amplitude.
...
- Scene Gallery selection persists through `/api/v1/experience/actions`: it uses `{ type:"set_scene" }` within the current room mode and `{ type:"set_mode", mode, sceneVideoId }` for a cross-mode selection. If Scene Sound is active, the audible `scene` source follows the newly selected video.
...
- The brightness/volume overlay can keep a Close affordance for explicit touch users, but it should not require a tap to recover the Ambient view; no further input for roughly three seconds closes it.
...
- `activeProvider` is the only fact for the left foreground pane. Its normal card label is `Current`; the selected treatment must not invent Spotify when runtime state is missing.
- During a switch, the old `activeProvider` remains `Current` and the target is `Opening`. A request accepted by `POST /api/v1/web-mode/actions` is only pending work, not proof that the target is foreground or usable.
- A background resident verified by the lifecycle is `Ready`; a provider with no resident record is `Waiting`; `prewarming`, `opening`, and `frozen` display `Prewarming`, `Opening`, and `Standby`. `check_setup`, `check_proxy`, regional unavailability, and reload failures take priority in the card label, while a failed foreground card retains its `Current` selected treatment.
- Every provider card has a separate accessible Reload control. It reloads the current provider in place; for another provider it waits for that target to become the actual foreground provider before reloading. The control locks repeat switch/reload requests, keeps Close available, retains the side panel, and reports a bounded reload failure that can be retried.
