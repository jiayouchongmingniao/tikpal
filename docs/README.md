...
- Player Library: storage tabs are flat (`Local`, `NAS`, `USB`, `Favorites`, `Recently Added`), Local/NAS/USB rows show file details, USB can copy to Local without overwriting same-name files, Local delete requires `Yes` / `No`, and long lists use the fixed right-side fast-scroll rail. SMB NAS setup lives in Settings -> Library -> NAS: discovery only suggests candidates, while `Test` and `Save & Scan` are required before a share is mounted and shown in the NAS tab. Changing between Local/NAS/USB is browse-only; MPD switches only after a track tap, using MPD-visible `Codex/...`, `NAS/...`, or `USB/...` paths and matching `Playing from ...` hints.
