...
Explore close returns immediately from the API, then restores Tikpal playback while the resident provider pool stays parked offscreen. The default visible close path favors fast return and instant reopen: it parks the side panel and provider windows, leaves provider guards running so inactive pages stay paused/muted, keeps the pool resident through `TIKPAL_WEB_MODE_CLOSE_KEEP_RESIDENT=1`, and avoids cold-starting a separate exit-stage Chromium. A boot-time warm-up can prelaunch the ten provider profiles offscreen after the kiosk is stable. Hot switching uses cached window ids and defaults to asynchronous X11 movement with `TIKPAL_WEB_MODE_X11_SYNC_WINDOW_OPS=0`; synchronous `xdotool --sync` is a compatibility fallback, not the Gentoo fast path. The optional neutral `web-mode-exit.html` room-return veil remains available through `TIKPAL_WEB_MODE_EXIT_VEIL_ENABLED=1`, but production Gentoo keeps it off so a slow Suno/YouTube/Apple page cannot leave the UI stuck on `Closing`.
