...
| `/api/v1/web-mode/actions` | `POST` | Opens, closes, or surfaces the Explore keyboard. `open` pauses Tikpal playback and launches the provider browser plus `/side-panel`; it does not write `audio.rememberedSource` or change `audio.currentSource`. The runtime keeps one visible provider window and retargets common new-window navigation into the left pane. |
| `/api/v1/web-mode/settings` | `PATCH` | Persists Explore proxy settings in `.tikpal/web-mode-settings.json`. The default development proxy is HTTP `http://192.168.10.140:7897`. |
