...
2026-07-19 audio-adapter validation on `192.168.31.110` deployed `tikpal-audio-adapt.service`, reset `.env.kiosk` audio outputs back to `auto`, and confirmed the adapter selected `SPL Crimson` by priority. `check` reported `browserPcm=plughw:CARD=Crimson,DEV=0`, `audiooutPcm=plughw:CARD=Crimson,DEV=0`, `mixerControl=none`, `volumeStrategy=mpd-software`, and `loopbackVisible=1`; Explore crossfade declined the non-`dmix` output and fell back to direct provider audio. A low-level `_audioout` WAV captured through `hw:Loopback,1,0` produced nonzero PCM with no new USB over-current events.
