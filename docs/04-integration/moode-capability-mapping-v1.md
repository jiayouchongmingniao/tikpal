...
| `/api/v1/experience/actions` | `POST` | Changes room modes, starts/stops timers, applies Hi-Fi EQ presets with `set_hifi_eq`, updates Scene Sound with `set_scene_sound`, and updates Auto Night schedule. Focus/Calm/Sleep default Scene Sound to off and switch to `scene` only when `sceneSoundEnabled: true` is explicit; Hi-Fi never switches to `scene`. |
