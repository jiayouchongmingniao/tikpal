...
`TIKPAL_AIRPLAY_METADATA_COMMAND` points to moOde's AirPlay metadata bridge. The checked-in `deploy/moode/tikpal-airplay-metadata.sh` reads legacy `/var/local/www/aplmeta.txt` when present, falls back to Shairport Sync MPRIS metadata on moOde 5, and can use `/var/local/www/aplmeta.json` when it is fresh. It emits title / artist / album / artwork fields that Tikpal can use for playback truth and LRCLIB lyrics lookup. `TIKPAL_AIRPLAY_METADATA_CLOCK_LEAD_MS` compensates for moOde's metadata write delay when Tikpal has to infer AirPlay progress from metadata mtime, while `TIKPAL_AIRPLAY_ARTWORK_MAX_LAG_SECONDS` prevents stale cover files from being paired with a newer title. `TIKPAL_AIRPLAY_ALSA_OUTPUT_DEVICE` defaults AirPlay to moOde's `_audioout` chain, so Shairport Sync reaches the physical output while the Loopback mirror remains available for the real Hi-Fi spectrum meter.
