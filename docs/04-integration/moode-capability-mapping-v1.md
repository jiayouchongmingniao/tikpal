...
- Explore controls in Console are limited to provider/proxy support. Its text fields use the shared automatic Onboard behavior; the compact manual keyboard toggle stays in the physical Explore side panel.
...
| `/api/v1/remote/actions` | `POST` | Single portable-controller write path protected by `X-Tikpal-Key`. It supports playback transport, seek, play mode, volume, source selection, room mode/session/timer, scene selection/sound, Hi-Fi EQ, display brightness, lyrics refresh, and `explore.open` / `explore.close` / `explore.proxy_set`. Explore open defaults to Spotify, close reuses the physical side panel's Back behavior, and proxy changes refresh the active provider without replacing its Chromium process/window/profile; the URL stays private. Direct remote `/api/v1/web-mode/*`, reboot, shutdown, library scan, and playlist CRUD remain blocked. |
