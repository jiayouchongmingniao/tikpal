...
- Hi-Fi uses the larger playback center controls: playback mode, source picker, previous/next, play/pause, favorite, playlist, and lyrics. Entering Hi-Fi restores the global remembered visible source from `audio.rememberedSource`: a saved Library track is retried with a plain Library fallback, a saved Radio station reopens by `radioStationId`, and saved Spotify/AirPlay/Bluetooth/DLNA sources reopen their waiting handoff state. Library memory follows the actual local song after manual selection, playlist play, next, and previous; Radio memory follows the final station that actually plays, even after the user returns to Library. Returning to either music source from Focus/Calm/Sleep should resume that source's last position without showing a blocking overlay.
