...
TIKPAL_WEB_MODE_EXIT_VEIL_ENABLED=1
TIKPAL_WEB_MODE_EXIT_VEIL_SETTLE_SECONDS=0.12
...
Provider profiles keep their login state. Before launching a provider, the launcher only repairs an empty or missing Widevine CDM directory by copying `WidevineCdm` from the first existing Chromium or provider profile that already has `libwidevinecdm.so`; it must not delete the provider profile to solve protected-playback failures such as TIDAL `S6001`.
