...
| `tikpal-web.service` | `4173`, `4174` | Loopback-only full Kiosk UI on `4173`; portable remote control on LAN-facing `4174`; both proxy `/api` to the API service with separate access boundaries. |
...
On a deployed Pi, open `http://<pi-ip>:4174/` for the portable remote control. For the full browser-rendered Kiosk UI from macOS, use the SSH tunnel helper and then open `http://127.0.0.1:4173/`; it shares backend playback, room, scene, artwork, and lyrics state with the physical kiosk without running VNC/noVNC.
...
`TIKPAL_MPD_STARTUP_VOLUME=30` is the startup anti-blast guard. When the API starts in `mpc` mode, Tikpal first tries to set MPD software volume to 30% and, when an output-volume helper is available, the physical output level to the same value before any startup restore or auto-resume path runs. This priming does not overwrite `.tikpal/audio-volume-state.json`'s remembered nonzero playback volume. Failures are logged as warnings and do not block startup.
...
`TIKPAL_AUDIO_ADAPT_MODE=auto` makes the Pi repair the active audio route at startup. Every consumer uses the same endpoint order: explicit `TIKPAL_AUDIO_CARD_FORCE`, then `TIKPAL_AUDIO_CARD_PRIORITY=BT66,Crimson`, then one USB playback endpoint when `TIKPAL_AUDIO_PREFER_SINGLE_USB=1`, then one non-HDMI playback endpoint, otherwise refusal with no guessed card. `resolve-browser`, `resolve-audioout`, and `resolve-hw` expose the same selection for Chromium, moOde/Everyday, and MPD Pure. The adapter writes moOde's selected card, `_audioout`, and Tikpal's Loopback mirror before MPD/API/kiosk start. The systemd unit also performs a short Loopback preflight before running the adapter, so a reboot where `systemd-modules-load` claims `snd_aloop` but `/proc/asound/cards` still lacks `Loopback` fails early instead of letting AirPlay or MPD silently open a broken `_audioout`. Use `deploy/moode/tikpal-audio-adapt.sh check` to see the selected card, hardware/browser/moOde PCMs, selection reason, mixer strategy, ALSA rate converter, and Loopback visibility.
`TIKPAL_ALSA_LOOPBACK_ALLOW_HDMI=0` keeps Tikpal from re-enabling a stale moOde ALSA Loopback override when `_audioout` routes only to HDMI. Most Tikpal installs should select the USB speaker or USB amplifier in moOde first, then let `_audioout` follow that current output while mirroring to Loopback for AirPlay, Bluetooth, and Hi-Fi spectrum. Set this to `1` only for an intentional HDMI-output install. For Chromium Scene Sound and Explore, leave `TIKPAL_CHROMIUM_ALSA_OUTPUT_DEVICE=auto` and `TIKPAL_WEB_MODE_ALSA_OUTPUT_DEVICE=auto` so the launcher asks the audio adapter for the current card's safest browser PCM. BT66 can use `dmix:CARD=BT66,DEV=0`; SPL Crimson uses Tikpal's generated `tikpal_browser_output` shared PCM, which wraps `dmix` in `plug` so Chromium's common 16-bit output is converted to the card's required 24-bit format without letting the kiosk and Explore provider block each other. `TIKPAL_ALSA_RATE_CONVERTER` is optional: when non-empty, Tikpal writes it only into its managed `_audioout` and shared-browser `plug` nodes; an empty value preserves ALSA's existing converter choice.
...
`TIKPAL_WEB_PORT=4173` is the loopback-only full Kiosk surface. `TIKPAL_WEB_REMOTE_PORT=4174` always injects the portable remote UI and keeps its proxy limited to `/api/v1/remote/*`. Use `TIKPAL_WEB_KIOSK_HOST=127.0.0.1` and `TIKPAL_WEB_REMOTE_HOST=0.0.0.0`; reach full Kiosk rendering from macOS through SSH forwarding instead of a LAN listener. The portable listener blocks remote access to the Kiosk API and cannot update `/api/v1/kiosk/heartbeat`.
`TIKPAL_SCENE_CONTEXT_GEO_*` and `TIKPAL_SCENE_CONTEXT_WEATHER_*` control the cached `/api/v1/scene/context` lookups used for weak Ambient clock copy. The endpoint prefers IP-derived timezone/location when available, falls back to the caller's `timeZone` query or the room-experience default when unavailable, and caches provider failures briefly so a slow network cannot stall normal state reads. Weather lookups request `temperature_2m` together with weather code and precipitation; Ambient shows rounded Celsius only when the provider returns it, otherwise the localized weather / room / daypart caption remains compact without an empty placeholder.
...
The generator writes packages under `.tikpal/resource-ota-packages` unless `--output` is provided. Split mode creates one package per MP4; `--bundle` creates one package that installs the whole folder together. Each generated scene entry includes an id, filename, label, order, optional default marker, and `sha256`. The generator normalizes scene MP4s for the Pi kiosk path: keep the physical `2560x720` target, use H.264 Main Profile Level 4.1, `yuv420p`, a closed GOP around 48 frames, no B-frames, bounded video bitrate around `4500k`, no audio stream, and `+faststart`.

Scene video is visual-only. Package a separate long Ogg asset, reference it through `audioFilename` / `audioSha256` / `audioGainDb`, and retain its source attribution in `scene_audio_sources.json`; use a versioned filename when its content changes. `FlameScene` preloads and crossfades video layers to soften visible loop boundaries, while `SceneAudioTransport` independently loops the Ogg without restarting on a video boundary, source change, or static-video fallback. The video watchdog can recover ordinary playback stalls and exposes `data-flame-video-health`; if the full X/Chromium/V3D display stack stops responding, the systemd kiosk watchdog restarts only `tikpal-kiosk.service`.
...
For Bluetooth, DLNA, and Radio parity, use the same cover / lyrics wall acceptance instead of treating AirPlay as a one-off. Bluetooth should report the active sender as `audio.currentSource.id:"bluetooth"` with `connectionState:"connected"`, real `playback.albumArtUrl` from BlueZ metadata or the cached remote artwork endpoint when available, and `lyrics.sourceScope:"bluetooth_input"` once metadata or fingerprint recognition identifies a track. DLNA should report `audio.currentSource.id:"upnp"` only after a real connected probe or new MPD playback appears after selecting DLNA, then expose title / artist / album / elapsed through `deploy/moode/tikpal-upnp-metadata.sh` with `lyrics.sourceScope:"upnp_input"` when metadata identifies a track. A real DLNA MPD stream with no usable DIDL metadata still belongs to `upnp_input` and falls back to audio fingerprinting; a configured Radio preset URI remains Radio when DLNA is only `armed` / `Ready`. Radio should report `audio.currentSource.id:"radio"`, the selected `radioStationId`, and a non-empty `playback.albumArtUrl` from `/api/v1/media/radio-logo`. Radio lyrics use `sourceScope:"local_playback"` and should only become `ready` when the stream exposes real song title / artist metadata; a station label such as `Internet Radio` with no track identity should stay centered now-playing with the station logo, not fake a lyrics wall.

### DLNA fingerprint capture

Trusted DIDL metadata queries lyrics immediately. When a connected DLNA stream lacks trustworthy metadata, Tikpal captures six seconds from a loopback-only MPD `httpd` tap and submits it to the configured ACRCloud provider. The same `upnp_input` lyrics state drives the existing cover-plus-lyrics wall and bottom ticker; there is no DLNA-only UI mode. A successful recognition is valid only for its current DLNA connection. The backend rechecks every `TIKPAL_UPNP_RECOGNITION_REFRESH_MS` (default `90000`) and replaces the wall only if the recognized track changes.

`deploy/systemd/install-systemd-services.sh` runs the guarded installer during deployment. It writes `Tikpal DLNA Recognition Tap` outside the managed MPD audio-profile block, binds it only to `127.0.0.1:8001`, leaves it disabled by default, and restarts MPD once on first installation. It enables `TIKPAL_UPNP_CAPTURE_COMMAND` only after `mpd`, `mpc`, `ffmpeg`, the MPD `httpd` plugin, and the installed output pass preflight. A failed preflight deliberately leaves fingerprint capture unset: DIDL lyrics remain available and no audio route or USB DAC output is changed.

After the first deployment, verify the installed but idle tap and one manual capture:
...
file "$capture"
rm -f "$capture"
ss -ltnp '( sport = :8001 )' # no listener after capture
```

Do this only while a real DLNA sender is playing. The capture command enables the tap for the bounded recording and disables it in all success, error, and signal paths. `mpc outputs` must continue to show the primary `Tikpal Pure Listening` output unchanged.
...
2026-07-19 Explore validation on `192.168.2.138` used the same SOCKS route, preserved Pi-local `.env*` and `.tikpal` during rsync, then ran `sudo deploy/systemd/install-systemd-services.sh --enable-kiosk --restart`. The installer filled the missing kiosk package set, including `xdotool`, `wmctrl`, `onboard`, `fcitx5`, `xvfb`, `x11vnc`, `novnc`, `websockify`, and `socat`. Runtime `.env.kiosk` kept `TIKPAL_CHROMIUM_ALSA_OUTPUT_DEVICE=auto` and `TIKPAL_WEB_MODE_ALSA_OUTPUT_DEVICE=auto`; the `--check` output resolved the current physical card as `dmix:CARD=BT66,DEV=0`, proving this was auto-detection rather than a checked-in device pin. That run predates the shared source compatibility helpers; current `.env` must use the Bluetooth and AirPlay disable helper paths rather than bare moOde renderer commands. `.tikpal/web-mode-settings.json` had `proxyEnabled=true` with `proxyUrl="http://192.168.2.172:7897"`. A follow-up kiosk check focused Console Link / Explore Proxy URL and verified Onboard at `1208,157 900,280`, leaving the input visible instead of covering it from the default bottom position.
...
Current Onboard script behavior is shared with the Gentoo path: `tikpalImeToggle.py` supports `--set-locale` and `--set-mode`, and the language key toggles `EN <-> selected UI language`. Pi package names and install steps differ from Gentoo, so use the Gentoo runbook only for Portage/Fcitx package details.
...
Set one non-empty `TIKPAL_PORTABLE_API_KEY` in the protected device `.env.kiosk`; both `tikpal-api.service` and `tikpal-web.service` must load that file. Check every later `EnvironmentFile` override, then restart both services. Port `4174` authorizes its LAN UI server-side and keeps no key field or browser-local key storage; action errors remain visible until another action clears them.
...
Scene Sound is browser audio from the independent looped Ogg asset declared by the active scene, not MPD or MP4 audio. Scene MP4s stay muted so an 8–30 second video loop or static-video fallback cannot restart ambient audio. If Scene Sound is on and the API reports `source:"scene"` but the speaker is silent, split the diagnosis into the `audio[data-scene-audio-transport]` element, Chromium ALSA output, moOde `_audioout`, and competing renderer ownership.
...
- If `librespot --device _audioout` is still running after the source is `scene`, run `./deploy/moode/tikpal-spotify-disable.sh`, set `TIKPAL_SPOTIFY_DISABLE_COMMAND="./deploy/moode/tikpal-spotify-disable.sh"`, and restart `tikpal-api.service`.
...
The USB PCM status should be `RUNNING` while Scene Sound is active, and recent kiosk logs should show the adapter-selected `dmix:CARD=...` or `plughw:CARD=...` output without new `PcmOpen` errors. If the DOM needs inspection, enable `TIKPAL_KIOSK_REMOTE_DEBUG=1` only temporarily, confirm every `video.flame-video` is muted, then inspect `audio[data-scene-audio-transport]` for the active `src`, `paused=false`, advancing `currentTime`, and nonzero `volume`; turn DevTools back off afterwards.
