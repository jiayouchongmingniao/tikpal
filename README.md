...
Explore is separate from that source model. It opens official music web players in a left Chromium window and a Tikpal `/side-panel` controller in a 640 x 720 right window; it pauses Tikpal playback before entry, does not write `rememberedSource`, and stores only Explore provider/proxy state in `.tikpal/web-mode-*.json`. The launcher keeps one visible provider window, retargets common new-window links into the same left pane, enables Chromium popup/ad blocking for the provider profile, and writes the active provider itself so the side panel highlight stays aligned with the actual left browser. The provider guard keeps Chromium errors and desktop-browser gestures out of the left pane. QQ Music prompts are conservative: dismiss buttons such as `取消` or `关闭` can be auto-clicked, but login, purchase, membership, authorization, agreement, recharge, and subscription confirmations stay manual. The development default proxy is HTTP, `http://192.168.10.140:7897`; Console Link exposes the proxy URL plus a Keyboard action that surfaces `onboard` for web-player login fields.
