...
- Scene Sound on Pi: the active browser video supplies scene audio, so Chromium should use `TIKPAL_CHROMIUM_ALSA_OUTPUT_DEVICE=auto` to detect the current physical USB `dmix` device; moOde `_audioout` / Loopback remains for MPD, renderer intakes, and spectrum capture.
