...
Remote proxy changes use the MV3 extension and refresh the active provider page while keeping its profile and window. The side panel status must read `Proxy On` or `Proxy Off`; do not use `Direct` as the visible state because direct mode still cannot reach several providers on this network. If a user explicitly selects a proxy-enabled provider whose resident page is Tikpal's local `check_proxy` friendly error, the foreground path probes the real provider URL once through the current Settings proxy before revealing anything. A successful probe navigates the existing CDP session back to the real HTTPS page and waits for `Ready`; an unreachable proxy, failed navigation, timeout, or repeat error leaves the friendly page at `Check proxy` without a loop, browser rebuild, or disruption to another active provider. Reconciliation can briefly show `Prewarming` while this page is parked, so the local friendly-error identity remains the retry gate. `region_unavailable` is terminal, and direct-preferred QQ Music and NetEase Cloud Music never use this retry path. The trace logs `proxy_retry` with provider, outcome, and elapsed time but not the proxy URL. Cookies and login state stay in per-provider Chromium profiles under:
...
Gentoo may launch Onboard through its Python console-script entry point, so the process name can be `python3` rather than `onboard`. The launcher therefore recognizes the installed `onboard` executable in the current user's command line before deciding to start, move, hide, toggle, or keep it alive. This prevents duplicate keyboards and false "not running" results while preserving the existing user service and DBus control path.
...
- The audio adapter bootstraps one `# Tikpal managed MPD library` block in `/etc/mpd.conf` before MPD starts. It enables the Tikpal music root, persistent MPD database and playlist paths, plus both MPD symlink-following options needed for `USB/<mount name>` links. The block is replaced idempotently and all unmarked MPD configuration remains untouched. Without this block, the UI can list mounted USB files but `mpc update` and track playback fail with `MPD error: No database`.
