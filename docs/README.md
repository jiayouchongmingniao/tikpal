...
- Room experience: startup offers Focus, Calm, Sleep, and Hi-Fi; Focus/Calm/Sleep open Scene Sound by default on boot, remember their own last selected scene, and selecting a music/input source keeps the scene video visible while muting scene audio. Hi-Fi applies `flat`, `warm`, or `vocal` EQ presets when the Pi command hook is configured, centers now-playing artwork and metadata, never enables Scene Sound, and restores `audio.rememberedSource` for the last Library track, Radio station, or external waiting source, including after API/kiosk restart while room mode is already Hi-Fi. `localTrackPath` follows the last actual local Library track, and `radioStationId` follows the last successful Radio station, so returning to either source can resume its own last position. Auto Night dims by selected timezone without changing sources.
- Hi-Fi lyrics wall: ready lyrics can replace centered now-playing with the shared cover-plus-lyrics wall. Its bottom footer uses a CSS-only mini EQ centered under the cover, a progress/time readout, and capability-gated playback controls; the mini EQ is an organic-looking deterministic simulation, not real spectrum data, and it must pause with playback.
