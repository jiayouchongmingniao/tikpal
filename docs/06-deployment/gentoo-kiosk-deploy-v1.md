...
- QQ Music ordinary playback uses `TIKPAL_WEB_MODE_QQ_AUDIO_PRIME=1` so the active page can gently open Chromium's ALSA path before QQ starts its own WebAudio stream. QQ Music MV uses wrapper cinema mode by default: `TIKPAL_WEB_MODE_QQ_MV_CINEMA_MODE=1`, `TIKPAL_WEB_MODE_QQ_MV_AUTO_PLAY=1`, and `TIKPAL_WEB_MODE_QQ_MV_AUTO_FULLSCREEN=0`.
