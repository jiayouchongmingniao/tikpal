...
The same `192.168.2.138` follow-up replaced the Onboard `中/EN` hotkey path with `tikpalImeToggle.py` because `xdotool key F9` and `Ctrl+Space` did not change Fcitx state while `fcitx5-remote -t` did. Physical Onboard clicks on `中/EN` were then verified to switch from `keyboard-us` / inactive to `pinyin` / active and back to `keyboard-us` / inactive.
