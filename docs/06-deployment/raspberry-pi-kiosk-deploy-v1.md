...
When lyrics exist, `/api/v1/lyrics/status` should be `ready` with `lines.length > 0`, `sourceScope: "airplay_input"`, and title / artist identity matching `/api/v1/playback/status`. If the provider only has a same-title different-artist result, `not_found` or continued `recognizing` is the correct state. If lyrics stay `idle` with a message like `Waiting for AirPlay audio`, first check `audio.currentSource.connectionState`; the lyrics wall intentionally stays hidden while AirPlay is only `armed`. If the wall shows the right song but the wrong highlighted line, compare helper `positionMs`, `positionTrusted`, playback `elapsedSeconds`, and the lyric line timestamps before changing provider logic. The Hi-Fi wall defaults visible through `tikpal.lyricsVisible.v3`; old hidden-state storage is restored visible once through `tikpal.lyricsVisible.autoRestored.v1`, then later manual hides remain respected. If a ready AirPlay lyric has no reliable active line because the raw position is missing, was inferred from metadata mtime after pause/resume, or was discarded as overrun, the wall should still show static rotating lyrics rather than wrapping elapsed time into the track and highlighting a false line.
