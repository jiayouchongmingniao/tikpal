...
`,
  ...
  assert(stylesSource.includes("width: min(840px, calc(100vw - 112px));") && stylesSource.includes("grid-template-columns: minmax(0, 1fr) repeat(4, auto);") && stylesSource.includes(".ambient-transport.is-room-mode .ambient-transport-mode-copy strong,") && stylesSource.includes("text-overflow: ellipsis;"), "Scene transport should keep its centered width and fixed control columns when playback titles vary");
