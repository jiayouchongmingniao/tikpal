...
- Link: network summary, System/API status, and Explore proxy/keyboard state.
...
- Explore controls in Console are limited to provider/proxy/keyboard support. Third-party music account login and playback stay inside the official web player window.
...
| `/api/v1/web-mode/actions` | `POST` | Opens, closes, or surfaces the Explore keyboard. `open` pauses Tikpal playback, keeps the existing `/side-panel`, and stages the target provider behind a left-screen transition; it does not write `audio.rememberedSource` or change `audio.currentSource`. Provider cards report `Ready`, rotating `Opening`, then stable `Active`. `close` does not wait on an unresponsive provider renderer. |
